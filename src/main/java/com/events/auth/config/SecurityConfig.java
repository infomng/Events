    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                .authorizeHttpRequests(auth -> auth
                        ...
                        .requestMatchers(GET, "/api/v1/products/**").hasAnyAuthority(Role.ADMIN.name(), Role.USER.name())
                        .requestMatchers(POST, "/api/v1/products/**").hasAnyAuthority(Role.ADMIN.name(), Role.USER.name())
                        .anyRequest()
                        .authenticated()
                )
